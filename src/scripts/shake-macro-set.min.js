...
!(function () {
    Macro.add('shake', {
        ...
        handler: function () {
            ...
            $wrapper
                .addClass('macro-' + this.name + ' ' + shakeClass)
    ...
    Macro.add(['shakescreen', 'shaketarget'], {
        handler: function () {
            ...
            if ('shakescreen' === this.name)
                ($targets = jQuery('#passages')),
                    (duration =
                        this.args.length > 0 ? this.args[0] : 1 / 0),
                ...
                (duration = this.args.length > 1 ? this.args[1] : 1 / 0),
                    (shakeClass =
                        'block' === $($targets[0]).css('display')
                            ? 'shake-block'
                            : 'shake');
            }
            if ('stop' === duration)
                return void $targets.removeClass(shakeClass);
            if (1 / 0 !== duration)
                try {
                    duration = Math.max(
                        Engine.minDomActionDelay,
